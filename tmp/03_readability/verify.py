...
for id, old in old_codes.items():
    new = new_codes[id]
    if "display(pd.DataFrame(workbook_connections(WORKBOOK))" not in old.source:
        assert ast.dump(ast.parse(old.source)) == ast.dump(ast.parse(new.source)), id
    assert old.outputs == new.outputs, id
    assert old.execution_count == new.execution_count, id
old_answers = re.findall(r'<!-- BEGIN SOLUTION -->.*?<!-- END SOLUTION -->', '\n'.join(c.source for c in before.cells), re.S)
new_answers = re.findall(r'<!-- BEGIN SOLUTION -->.*?<!-- END SOLUTION -->', '\n'.join(c.source for c in nb.cells), re.S)
assert old_answers == new_answers
print('Model code and existing outputs preserved; only reference display/comment changed. Answers unchanged.', flush=True)

...
def reconstruction_sheet():
    """Box outlines only: students reconstruct transfers and inventory effects."""
    c = Canvas()
    c.text(30, 42, '03.1   Reconstruct the Boudreau-like model', 32, bold=True)
    c.text(30, 80, 'Student worksheet: add states, boundaries, arrows and descriptive process IDs.', 22)
    c.text(30, 112, 'Use the corrected reading and Excel inputs in notebook 03. Repeated boxes are the same reservoirs.', 19, MUTED)
    for x, title in ((30, 'A   Water transport and gas exchange'),
                     (785, 'B   Biological and carbonate processes')):
        c.rect(x, 145, 745, 685, white, HexColor('#BCCAD3'))
        c.text(x+18, 181, title, 23, bold=True)
    c.box(90, 222, 615, 78, 'Atmosphere [CO2_At]', 'State and inventory conversion:')
    c.box(90, 434, 240, 100, 'Low latitude [L_b]', 'States:')
    c.box(465, 434, 240, 100, 'High latitude [H_b]', 'States:')
    c.box(90, 688, 615, 92, 'Deep ocean [D_b]', 'States:')
    c.box(820, 310, 275, 95, 'Low latitude [L_b]', 'Same reservoir as panel A')
    c.box(820, 682, 275, 95, 'Deep ocean [D_b]', 'Same reservoir as panel A')
    c.rect(1195, 465, 310, 145, HexColor('#FFF6EA'), ORANGE)
    c.text(1210, 497, 'Carbonate / sediment', 23, bold=True)
    c.text(1210, 530, 'process module', 23, bold=True)
    c.text(1210, 565, 'Snowline is a memory state;', 18)
    c.text(1210, 592, 'no explicit sediment C stock.', 18)
    c.text(820, 234, 'Place weathering and net burial at the chosen boundary.', 19, MUTED)
    c.rect(30, 865, 1500, 222, HexColor('#EDF5FF'), BLUE)
    for y, line in zip((900, 940, 980, 1020, 1060), (
        '1. Separate circulation legs and the two mixing directions. Indicate the source concentration.',
        '2. Draw invasion and outgassing at each surface. Mark material arrows and information links differently.',
        '3. Show POC, PIC, dissolution, weathering and signed net burial. Do not add an extra carbon reservoir.',
        '4. Complete the companion flux table: endpoints, inventory effects, rule, units and status.',
        '5. Reconcile your specification with Excel, then use it for native construction. No drawing software is required.',
    )):
        c.text(48, y, line, 21)
    c.text(30, 1140, 'T_* = circulation; M_* = mixing; G_L/G_H = gas exchange. Use POC, PIC, W, D and B_net for the other processes.', 19, MUTED)
    c.text(30, 1180, 'Blank space is for your arrows and notes. A sketch on paper is equally suitable.', 21)
    c.text(30, 1240, 'Source: course adaptation of Wortmann et al. (2025), Fig. 3. Read the corrections in notebook 03.', 18, MUTED)
    return c.d
...
def diagram(student, p):
    ...
    rows = [
        ...
        ('W', f"1 DIC : 2 TA; {p['weathering_dic']:g} Tmol C/yr (lumped)", 'weathering_dic; TA = 2 x carbon input', 'weathering; Fw to L_b'),

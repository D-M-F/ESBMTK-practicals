...
piston-velocity comparisons. Reuse 00's input-pair and result-extraction skills,
...
units, plus three synthesis sentences on the mismatch, calibration and equilibrium
controls. Mask the whole calculation, including input types, solver call and TA
...
whether any represented process can change it. One final synthesis replaces the
separate boundary and completion questions; real-ocean TA sources are supplied
context, not an extra recall task. The connection-summary call is removed.
...
00. No pump attribution, OA/OAE time histories or sediment analysis here.
...
provisional 35-minute pilot rather than claiming verified completion time.
...
runs. These clarify the same tasks and two derivations; scope, prerequisites and
the provisional 20/20/15-minute allocation are unchanged. Include the revised
reading in the novice-student timing pilot.
